...
import { Flex, Text } from '@tallo/core-ui-library';
...
export const Avatar = ({
  ...
  const renderBadge = () => {
    if (!_.isEmpty(badgeProps)) {
      return (
        <Badge testID={`${testID}.onlineBadge`} {...badgeProps} size={badgeSize} containerStyle={getBadgePosition} />
      );
    }
  };
  ...
  return (
    <Container style={[getContainerStyle(), containerStyle]} testID={testID}>
      <View testID={`${testID}.container`} style={textContainerStyle}>
        {!_.isUndefined(text) && (
          <Text textType="headerBold2" numberOfLines={1} testID={`${testID}.label`}>
            {text}
          </Text>
        )}
      </View>
