...
export const Container = React.memo(
  React.forwardRef<CollapsibleRef, CollapsibleProps>(
    (
      ...
      const scrollY: ContextType['scrollY'] = useSharedValue(Object.fromEntries(tabNamesArray.map((n) => [n, 0])));

      const contentHeights: ContextType['contentHeights'] = useSharedValue(tabNamesArray.map(() => 0));

      const tabNames: ContextType['tabNames'] = useDerivedValue<TabName[]>(() => tabNamesArray, [tabNamesArray]);
      ...
      const indexDecimal: ContextType['indexDecimal'] = useSharedValue(index.value);
       ...
       * */
      const toggleSyncScrollFrame = (toggle: boolean) => syncScrollFrame.setActive(toggle);
