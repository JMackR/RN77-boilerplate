import { useMemo, Children, useState, useCallback, useContext, MutableRefObject, useEffect, useRef } from 'react';
...
export function useAnimatedDynamicRefs(): [ContextType['refMap'], ContextType['setRef']] {
    ...
    if (children) {
      Children.forEach(children, (element, index) => {
        ...
        if (element.type !== tabType)
          throw new Error('Container children must be wrapped in a <Tabs.Tab ... /> component');
...
export function useCollapsibleStyle(): CollapsibleStyle {
  const { headerHeight, tabBarHeight, containerHeight, width, allowHeaderOverscroll, minHeaderHeight } =
    useTabsContext();
...
export function useScroller<T extends RefComponent>() {
  ...
  const scroller = useCallback(
    (ref: AnimatedRef<T> | undefined, x: number, y: number, animated: boolean, _debugKey: string) => {
      'worklet';
      if (!ref) return;
 ...
 */
export function useSharedAnimatedRef<T extends RefComponent>(outerRef: ForwardRefType<T>) {
...
export function useAfterMountEffect(nextOnLayout: ViewProps['onLayout'], effect: React.EffectCallback) {
...
export function useConvertAnimatedToValue<T>(animatedValue: Animated.SharedValue<T>) {
