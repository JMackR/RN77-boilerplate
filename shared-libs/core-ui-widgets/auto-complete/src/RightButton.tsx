import { CancelIcon, DownDragIcon } from '@tallo/assets';
      ...
      >
        {!loading && showClear && (
          <TouchableOpacity onPress={onClearPress} style={styles.clearButton}>
            {ClearIconComponent ?? <SVG localSVG={{ SVG: CancelIcon.SVG, size: { height: 20, width: 20 } }} />}
          </TouchableOpacity>
        )}
        ...
        {showChevron && (
          <Animated.View style={{ transform: [{ rotate: chevronSpin }] }}>
            <TouchableOpacity onPress={onChevronPress} style={styles.chevronButton}>
              {ChevronIconComponent ?? (
                <SVG iconTint={'brand'} localSVG={{ SVG: DownDragIcon.SVG, size: { height: 20, width: 20 } }} />
              )}
            </TouchableOpacity>
          </Animated.View>
        )}
