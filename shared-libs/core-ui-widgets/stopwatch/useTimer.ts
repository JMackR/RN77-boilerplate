...
import { setElapsedTime } from '@tallo/training/src/provider/training.slice';
...
const useTimer = ({
  ...
  }, [elapsedInMs, initialTimeInMs, mode]);
  ...
  function play() {
    ...
    // console.log('play', elapsedInMs, initialTimeInMs, mode);
    ...
    intervalId.current = setInterval(() => {
      ...
      if (!pausedTime.current) {
        // console.log('set Elapsed time', Date.now() - startTime.current!);
        setElapsedInMs(Date.now() - startTime.current!);
        dispatch(setElapsedTime(Date.now()))
      } else {
  ...
  return {
    ...
    hours: Math.floor(countInSeconds / 60 / 60),
    play,
    pause,
    reset,
    getSnapshot,
