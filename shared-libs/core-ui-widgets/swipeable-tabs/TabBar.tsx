...
import { type Props as IndicatorProps, TabBarIndicator } from './TabBarIndicator';
...
const getFlattenedPaddingStart = (style: StyleProp<ViewStyle>) => {
  ...
  return flattenStyle
    ? flattenStyle.paddingLeft || flattenStyle.paddingStart || flattenStyle.paddingHorizontal || 0
    : 0;
};
...
const convertPaddingPercentToSize = (value: DimensionValue | undefined, layout: Layout): number => {
  switch (typeof value) {
    case 'number':
      return value;
    case 'string':
...
const getMaxScrollDistance = (tabBarWidth: number, layoutWidth: number) => tabBarWidth - layoutWidth;

const getTranslateX = (scrollAmount: Animated.Value, maxScrollDistance: number, direction: LocaleDirection) =>
  Animated.multiply(
...
const renderIndicatorDefault = (props: IndicatorProps<Route>) => <TabBarIndicator {...props} />;
...
export function TabBar<T extends Route>({
  ...
  const [layout, setLayout] = React.useState<Layout>(propLayout ?? { width: 0, height: 0 });
  ...
  const hasMeasuredTabWidths =
    Boolean(layout.width) && routes.slice(0, navigationState.index).every((r) => typeof tabWidths[r.key] === 'number');
  ...
  const translateX = React.useMemo(
    () => getTranslateX(scrollAmount, getMaxScrollDistance(tabBarWidth, layout.width), direction),
    ...
    ({ item: route, index }: ListRenderItemInfo<T>) => {
      ...
      const onLayout = isWidthDynamic
        ? (e: LayoutChangeEvent) => {
            ...
            if (
              ...
              routes.slice(0, MEASURE_PER_BATCH).every((r) => typeof measuredTabWidths.current[r.key] === 'number')
            ) {
      ...
      return (
        <>
          ...
          {renderTabBarItem ? renderTabBarItem(props) : <TabBarItem {...props} />}
        </>
      );
    },
    [
  ...
  const contentContainerStyleMemoized = React.useMemo(
    () => [styles.tabContent, scrollEnabled ? { width: tabBarWidth } : null, contentContainerStyle],
  ...
  const handleViewableItemsChanged = useLatestCallback(({ changed }: { changed: ViewToken[] }) => {
    ...
    <Animated.View onLayout={handleLayout} style={[styles.tabBar, style]}>
      ...
      >
        {renderIndicator({
          ...
          style: [indicatorStyle, { start: flattenedPaddingStart, end: flattenedPaddingEnd }],
