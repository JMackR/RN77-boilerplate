...
const getActiveOpacity = (position: Animated.AnimatedInterpolation<number>, routesLength: number, tabIndex: number) => {
  if (routesLength > 1) {
...
type TabBarItemInternalProps<T extends Route> = Omit<
  Props<T>,
  'navigationState' | 'getAccessibilityLabel' | 'getLabelText' | 'getTestID' | 'getAccessible' | 'options'
> & {
  isFocused: boolean;
  index: number;
  routesLength: number;
...
const TabBarItemInternal = <T extends Route>({
  ...
  const tabContainerStyle: ViewStyle | null = isWidthSet ? null : { width: defaultTabWidth };
  ...
  return (
    ...
    >
      <View pointerEvents="none" style={[styles.item, tabStyle]}>
        {icon}
        <View>
          <Animated.View style={{ opacity: inactiveOpacity }}>{renderLabel(false)}</Animated.View>
          <Animated.View style={[StyleSheet.absoluteFill, { opacity: activeOpacity }]}>
            {renderLabel(true)}
          </Animated.View>
        </View>
        {customBadge != null ? <View style={styles.badge}>{customBadge({ route })}</View> : null}
      </View>
    </PlatformPressable>
  );
};

const MemoizedTabBarItemInternal = React.memo(TabBarItemInternal) as typeof TabBarItemInternal;

export function TabBarItem<T extends Route>(props: Props<T>) {
  const { onPress, onLongPress, onLayout, navigationState, route, ...rest } = props;
