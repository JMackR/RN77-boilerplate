...
export function TabBarIndicator<T extends Route>({
  ...
  if (Platform.OS === 'web' && width === 'auto') {
    styleList.push({ width: transform[1].scaleX }, { left: transform[0].translateX });
  } else {
    styleList.push(
  ...
  return (
    <Animated.View style={[styles.indicator, styleList, width === 'auto' ? { opacity: opacity } : null, style]}>
      {children}
    </Animated.View>
  );
}
